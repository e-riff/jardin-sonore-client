...
final class Theme implements IdentifiableInterface, UuidIdentifiableInterface
{
    use IdentifiableTrait;
    use UuidIdentifiableTrait;

    public function __construct(private string $label, private string $color = '#64748b', ?Uuid $uuid = null, ?int $id = null)
    {
    ...
    {
        if ('' === trim($label)) {
            throw new InvalidArgumentException('Theme label cannot be blank.');
    ...
    public function changeColor(string $color): void
    {
        ...
        if (!preg_match('/^#[0-9a-fA-F]{6}$/', $color)) {
            throw new InvalidArgumentException('Theme color must be a hexadecimal color.');
