...
final class Version20260623143000 extends AbstractMigration
{
    ...
    private function normalizePhoneNumbersSql(string $tableName, string $columnName): string
    {
        ...
        $spaced = "TRIM(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(REPLACE({$columnName}, '.', ' '), '-', ' '), '(', ' '), ')', ' '), '[[:space:]]+', ' '))";

        return <<<SQL
UPDATE {$tableName}
SET {$columnName} = CASE
    WHEN {$digits} REGEXP '^0[0-9]{9}$' THEN CONCAT(SUBSTRING({$digits}, 1, 2), ' ', SUBSTRING({$digits}, 3, 2), ' ', SUBSTRING({$digits}, 5, 2), ' ', SUBSTRING({$digits}, 7, 2), ' ', SUBSTRING({$digits}, 9, 2))
    WHEN {$digits} REGEXP '^0033[0-9]{9}$' THEN CONCAT('+33 ', SUBSTRING({$digits}, 5, 1), ' ', SUBSTRING({$digits}, 6, 2), ' ', SUBSTRING({$digits}, 8, 2), ' ', SUBSTRING({$digits}, 10, 2), ' ', SUBSTRING({$digits}, 12, 2))
    WHEN {$digits} REGEXP '^\\+33[0-9]{9}$' THEN CONCAT('+33 ', SUBSTRING({$digits}, 4, 1), ' ', SUBSTRING({$digits}, 5, 2), ' ', SUBSTRING({$digits}, 7, 2), ' ', SUBSTRING({$digits}, 9, 2), ' ', SUBSTRING({$digits}, 11, 2))
    ELSE {$spaced}
END
