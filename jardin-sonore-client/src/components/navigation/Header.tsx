        ...
        <header className={`fixed left-0 top-0 z-50 w-full border-b transition duration-300 ${scrolled ? "border-outline-variant/40 bg-background/88 py-2 backdrop-blur-xl" : "border-transparent bg-background/72 py-3 backdrop-blur-md"}`}>
            <nav className="mx-auto max-w-320 px-6 sm:px-margin" aria-label={content.ariaLabel}>
                ...
                <div className={`${menuOpen ? "grid" : "hidden"} gap-2 pb-3 pt-5 md:hidden`} id="mobile-menu">
                    {content.navigation.map((item: LinkItem) => (
                        <a
                            className="rounded-xl px-4 py-3 font-sans text-sm font-semibold tracking-[0.05em] text-on-surface-variant transition hover:bg-primary/10 hover:text-primary"
                            href={item.href}
                            key={item.href}
                    ...
                    <a
                        className="mt-2 inline-flex w-full items-center justify-center rounded-full border border-primary bg-primary px-6 py-3 font-sans text-sm font-bold tracking-[0.05em] text-on-primary soft-shadow transition duration-200 hover:-translate-y-0.5 hover:bg-primary-container"
                        href="#contact"
                        onClick={() => setMenuOpen(false)}
                    >
                        {content.reserveCta}
