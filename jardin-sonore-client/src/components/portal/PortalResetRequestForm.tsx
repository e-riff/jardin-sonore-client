...
import {idlePortalFormState, requestPortalPasswordResetAction} from "@/app/portail/actions";
import {useTranslations} from "@/i18n/translations-provider";

export default function PortalResetRequestForm(): JSX.Element {
    const [state, formAction, pending] = useActionState(requestPortalPasswordResetAction, idlePortalFormState);
    const content = useTranslations().portal.reset;

    if (state.status === "reset-confirmed") {
        return <p className="rounded-2xl bg-secondary-container px-5 py-4 font-sans text-sm leading-6 text-on-secondary-container" role="status">{content.confirmation}</p>;
    }
